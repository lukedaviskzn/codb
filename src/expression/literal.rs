...
use crate::{db::{registry::{Registry, TTypeId}, relation::Relation, DbRelations}, expression::Expression, typesystem::{scope::{ScopeTypes, ScopeValues}, ttype::{ArrayType, EnumType, StructType, TType}, value::{ArrayValue, EnumValue, ScalarValue, StructValue, Value}, TypeError, TypeSet}};
...
impl Literal {
    ...
    pub fn eval_types<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeTypes) -> Result<TTypeId, TypeError> {
        match self {
            Literal::Composite(literal) => literal.eval_types(registry, relations, scopes),
            Literal::Scalar(literal) => Ok(literal.ttype_id()),
            Literal::Array(literal) => literal.eval_types(registry, relations, scopes),
        }
    }

    pub fn eval<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeValues) -> Result<Value, EvalError> {
        match self {
            Literal::Composite(literal) => literal.eval(registry, relations, scopes),
            Literal::Scalar(literal) => Ok(literal.clone().into()),
            Literal::Array(literal) => literal.eval(registry, relations, scopes),
        }
    }
}
...
impl CompositeLiteral {
    pub fn eval_types<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeTypes) -> Result<TTypeId, TypeError> {
        match self {
            CompositeLiteral::Struct(literal) => literal.eval_types(registry, relations, scopes),
            CompositeLiteral::Enum(literal) => literal.eval_types(registry, relations, scopes),
        }
    }

    pub fn eval<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeValues) -> Result<Value, EvalError> {
        match self {
            CompositeLiteral::Struct(literal) => literal.eval(registry, relations, scopes),
            CompositeLiteral::Enum(literal) => literal.eval(registry, relations, scopes),
        }
    }
}
...
impl StructLiteral {
    ...
    pub fn eval_types<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeTypes) -> Result<TTypeId, TypeError> {
        ...
        for (field_name, field_value) in &self.fields {
            ...
            if let Some(field_ttype_id) = ttype.fields().get(field_name) {
                let field_value_ttype = field_value.eval_types(registry, relations, scopes)?;
    ...
    pub fn eval<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeValues) -> Result<Value, EvalError> {
        ...
        for (ident, expr) in &self.fields {
            fields.insert(ident.clone(), expr.eval(registry, relations, scopes)?);
        }
...
impl EnumLiteral {
    ...
    pub fn eval_types<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeTypes) -> Result<TTypeId, TypeError> {
        ...
        if let Some(tag_ttype_id) = ttype.tags().get(&tag_name) {
            let value_ttype_id = self.value.eval_types(registry, relations, scopes)?;
    ...
    pub fn eval<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeValues) -> Result<Value, EvalError> {
        ...
        let enum_value = unsafe { EnumValue::new_unchecked(
            self.ttype_id.clone(),
            self.tag.clone(),
            self.value.eval(registry, relations, scopes)?,
        ) };
...
impl ArrayLiteral {
    ...
    pub fn eval_types<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeTypes) -> Result<TTypeId, TypeError> {
        ...
        for entry in &self.entries {
            let entry_ttype_id = entry.eval_types(registry, relations, scopes)?;
    ...
    pub fn eval<R: Relation>(&self, registry: &Registry, relations: &DbRelations<R>, scopes: &ScopeValues) -> Result<Value, EvalError> {
        ...
        for entry in &self.entries {
            entries.push(entry.eval(registry, relations, scopes)?);
        }
